...
2. Inspect the staged changes and write a concise Conventional Commit message that accurately summarizes them.
...
- Use Conventional Commits format: \`type(scope): description\` or \`type: description\`.
- Choose a type: \`feat\`, \`fix\`, \`docs\`, \`style\`, \`refactor\`, \`perf\`, \`test\`, \`build\`, \`ci\`, \`chore\`, \`revert\`.
- Keep the commit message minimal and concise.
- Prefer a single-line commit message.
- Use a multi-line commit message only when the staged changes are large or complex, keep the subject line concise;
  put only necessary context in the body.`;
