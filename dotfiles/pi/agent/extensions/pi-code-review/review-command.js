...
async function getChangedFiles(pi, cwd, options) {
...
async function readFileContent(pi, cwd, filePath) {
    const result = await pi.exec("cat", [filePath], { cwd });
    if (result.code === 0)
        return result.stdout;
    return "";
}
export async function handleReviewCommand(args, ctx, pi) {
    ...
    if (filePaths.length > MAX_FILES) {
        ctx.ui.notify(`Reviewing first ${MAX_FILES} of ${filePaths.length} changed files.`, "info");
    }
    const editedFiles = capped.map((path) => ({
        path,
    ...
    if (apiKey) {
        try {
            const filesWithContent = await Promise.all(editedFiles.map(async (f) => ({
                ...f,
            ...
            pi.sendUserMessage(`Code review complete for ${capped.length} file(s):\n\n${formatted}`, { deliverAs: "followUp" });
            return;
        }
        catch (err) {
